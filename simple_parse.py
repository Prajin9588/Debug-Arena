...
for i, line in enumerate(lines):
    if i < 5: print(f"Check line {i}: {repr(line)}")
...
for i in range(len(q_starts)):
    ...
    # parse header: Q1 — Title
    parts = header_line.split(' ', 2) # Q1, -, Title...
    q_id = parts[0]
    # title is the rest
    # find first alpha char for title
    title_start = 0
    ...
    for line in body_lines:
        ...
        for h in headers_map:
            if line_stripped.startswith(h) and len(line_stripped) <= len(h) + 5: # rough check
                if current_header:
        ...
        if not is_header:
            if current_header:
                current_content.append(line.rstrip()) # keep indentation? User code has indentation.
